...
@Slf4j
public class ChosungSearchQuery {
     ...
     */
    public static String makeQuery(String strSearch, String column){
...
//                    bChosung = true;
            ...
            if( StartUnicode > 0 && EndUnicode > 0 ){
                if( StartUnicode == EndUnicode )
                    query.append("substr("+column+","+(nIndex+1)+",1)='"+strSearch.charAt(nIndex)+"'");
                else
                    query.append("(substr("+column+","+(nIndex+1)+",1)>='"+convertUnicodeToChar(StartUnicode)
                            +"' AND substr("+column+","+(nIndex+1)+",1)<'"+convertUnicodeToChar(EndUnicode)+"')");
            }
            else{
                if( Character.isLowerCase(strSearch.charAt(nIndex))){ //영문 소문자
                    query.append("(substr("+column+","+(nIndex+1)+",1)='"+strSearch.charAt(nIndex)+"'"
                            + " OR substr("+column+","+(nIndex+1)+",1)='"+Character.toUpperCase(strSearch.charAt(nIndex))+"')");
                }
                else if( Character.isUpperCase(strSearch.charAt(nIndex))){ //영문 대문자
                    query.append("(substr("+column+","+(nIndex+1)+",1)='"+strSearch.charAt(nIndex)+"'"
                            + " OR substr("+column+","+(nIndex+1)+",1)='"+Character.toLowerCase(strSearch.charAt(nIndex))+"')");
                }
                else //기타 문자
                    query.append("substr("+column+","+(nIndex+1)+",1)='"+strSearch.charAt(nIndex)+"'");
            }
        ...
        if(query.length() > 0 && strSearch != null && strSearch.trim().length() > 0) {
            ...
            if(strSearch.indexOf(" ") != -1) {
                ...
                for(int i=0, isize=tokens.length; i<isize; i++) {
                    ...
                    retQuery.append(column+" like '%"+token+"%'");
                }
                retQuery.append(")");
            } else {
                // LIKE 검색 추가
                retQuery.append(" OR "+column+" like '%"+strSearch+"%'");
            }
        } else {
            retQuery.append(query.toString());
    ...
    public static String searchSql(String searchStr, String columnName) {
        return ChosungSearchQuery.makeQuery(searchStr, columnName);
    }


    @Test
    public void serachQueryTest(){
        String sql = "SELECT * FROM ACCOUNT";
        sql += " WHERE ";
        sql += ChosungSearchQuery.makeQuery("ㅁㅂㅅㄴㅁ", "nick_name");
        log.info(sql);
    }
