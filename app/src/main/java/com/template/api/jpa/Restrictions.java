...
public class Restrictions {
    ...
    public Restrictions ilike(String key, String object) {
        conditions.add(new Condition(ConditionType.ILIKE, key, object));
        return this;
    }

    public Restrictions nlike(String key, String object) {
        conditions.add(new Condition(ConditionType.NOT_LIKE, key, object));
        return this;
    }

    public Restrictions nilike(String key, String object) {
            ...
            public Predicate toPredicate(Root<T> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
                ...
                for (Condition condition : conditions) {
                    ...
                    switch (condition.type) {
                        ...
                        case ILIKE: {
                            items.add(cb.like(cb.lower(getPath(root, key)), ((String) object).toLowerCase()));
                            break;
                        }
                        case NOT_LIKE: {
                            items.add(cb.not(cb.like(getPath(root, key), (String) object)));
                            break;
                        }
                        case NOT_ILIKE: {
                            items.add(cb.not(cb.like(cb.lower(getPath(root, key)), ((String) object).toLowerCase())));
                            break;
                        }
                        case IS_NULL: {
                        ...
                        case ARRAY_CONTAINS_FUNCTION: {
                            Expression fn = cb.function("array_contains", Boolean.class, root.get(key), cb.literal((String) object));
                            items.add(cb.equal(fn, true));
                            break;
                        }
